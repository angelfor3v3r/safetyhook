...
Expected<VmtHook, VmtHook::Error> VmtHook::create(void* object) {
    VmtHook hook{};

    const auto original_vmt = *static_cast<uint8_t***>(object);
    ...
    if (!allocation) {
        return Unexpected{Error::bad_allocation(allocation.error())};
    }
